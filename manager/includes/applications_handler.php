...
try {
    ...
    $notificationFile = '../../config/notification_helper.php';
...
// Authorization check - only Manager role can access
    ...
    try {
        ...
        $formattedApplications = array_map(function($app) {
            ...
            if ($paymentProofPath) {
                ...
                // Create a URL that uses our file viewer
                // Format: includes/file_viewer.php?file=filename.pdf
                $paymentProofPath = 'includes/file_viewer.php?file=' . urlencode($filename);
            }
...
function approveApplication($pdo, $employeeId) {
    ...
    try {
        ...
        // Update registration status to "Verified" to mark that manager approved it
        ...
        // Send verification email with registration ID
        $memberName = $registration['first_name'] . ' ' . $registration['last_name'];
        $verificationUrl = 'https://' . $_SERVER['HTTP_HOST'] . '/empirefitness/verify-email.php?registration_id=' . urlencode($registrationId);
        $emailResult = sendApprovedApplicationEmail($registration['email'], $memberName, $verificationUrl, $registrationId);
        
        // Save approval action to file for records
        saveApprovalActionToFile($registration, 'approved', $registrationId);
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                ...
                <h3>Next Steps:</h3>
                <ol>
                    <li>Click the button below to log in to your member account</li>
                ...
                <div style='text-align: center; margin: 30px 0;'>
                    <a href='http://localhost/pbl_project/login/log_in_page.html' style='display: inline-block; background: #dc2626; color: white; padding: 12px 40px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 16px;'>🔓 Login to Your Account</a>
                </div>
