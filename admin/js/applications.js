...
function displayPaymentProof(proofPath) {
    ...
    if (ext === 'pdf') {
        container.innerHTML = `
            <div class="payment-proof-container">
                <div class="payment-proof-pdf">
                    ...
                    <div>
                        <a href="${proofPath}" target="_blank" style="color: #d41c1c; text-decoration: none;">
                            <i class="fas fa-external-link-alt"></i> View PDF Payment Proof
                        </a>
                        <p style="margin: 5px 0 0 0; font-size: 12px; color: #6c757d;">Click to open in new tab</p>
                    </div>
                </div>
            </div>
        `;
        ...
        container.innerHTML = `
            <div class="payment-proof-container">
                <img src="${proofPath}" alt="Payment Proof" class="payment-proof-image" onclick="openImageModal('${proofPath}')" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'no-payment-proof\\'>Image failed to load</div>';">
...
async function quickApprove(id) {
    ...
    try {
        ...
        if (data.success) {
            showAlert(`✅ Application Approved!\n\nUsername: ${data.username}\nTemporary Password: ${data.temporary_password}\n\n${data.message}`, 'success');
...
async function approveApplication() {
    ...
    if (!confirm(`Approve registration for ${app.firstName} ${app.lastName}?\n\nA verification email will be sent to confirm their email address.`)) return;
    ...
    try {
        ...
        if (data.success) {
            ...
            showAlert(`✅ Application Approved!\n\nVerification email sent to: ${app.email}\n\nThe member will receive their login credentials after verifying their email address.\n\n${data.message}`, 'success');
            await loadApplications();
            await loadStats();
        } else {
            showAlert('Error: ' + data.message, 'error');
